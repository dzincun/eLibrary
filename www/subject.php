...
<?php
	...
	WHERE subject.subject_id = '$id' 
	 ...
	 AND book_details.book_id = saved_author.book_id 
	 AND soft_copy <> '' LIMIT 30") or die(mysql_error());
	 $count=mysql_num_rows($getcat);

if($count>=1){
	echo "<table border=1 cellspacing=0 cellpadding=3 style='width:680px;'>";
	echo "<tr><td width=40>№</td><td>Название</td><td>Автор(ы)</td><td width=70>ISBN</td></tr>";

	while($rows = mysql_fetch_array ($getcat)){
		$title = $rows['title'];
		$isbn = $rows['isbn'];
		$author_name = $rows['author_name'];
		$link = '/books/'.$rows['soft_copy'];
		$book_id2 = $rows['book_id'];
		echo "<tr><td>$sn</td>
				<td>
				$title
				<form name='submitForm' method='POST' action='loadBook.php'>
				<input type='hidden' name='book' value='$book_id2'>
