...
if(confirm("Вы действительно собираетесь удалить данную категорию?")){
return true;
}
else{
return false;
...
if(confirm("Вы действительно собираетесь начать редактирование данной категории?")){
return true;
}
else{
return false;
...
echo "<tr><td>№</td><td> Категория </td><td>Действие 1</td><td>Действие 2</td></tr>";
while($row=mysql_fetch_array($result)){
$subject_id= $row['subject_id'];		
$subject_name= $row['subject_name'];

//if 	($subject_name[0]==$test || $subject_name[0] == $test2){
 echo "<tr><td>$sn</td><td>$subject_name</td><td><form  action=edit_subject.php onSubmit=return(readySubmit2())><input type=hidden name=id value=$subject_id><input type=submit value=Редактирование ></form></td><td><form  action=delete_subject.php onSubmit=return(readySubmit())><input type=hidden name=id value=$subject_id><input type=submit value=Удалить ></form></td></tr>";
 $sn++;
//}
