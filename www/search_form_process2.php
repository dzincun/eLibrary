...
if($_GET['option']=='title'){
	...
	//$sn=1;
	if($count==0){
		echo "<li> Книг с данным названием не найдено. Попробуйте снова. </li>";}
	else{
		echo "<h1>Книги по названию $criteria</h1>";
		echo "<table border=1 cellspacing=0 cellpadding=3 style='width:680px;'>";
		echo "<tr><td>№</td><td>Название</td><td>Публикатор</td><td>Автор(ы)</td></tr>";
		$no = 0;
		while($row=mysql_fetch_array($result)){
			...
			echo "<tr valign='top'><td>$no</td><td>$title&nbsp;";
...
elseif ($_GET['option'] == "author"){
	...
	$sn=1;
	if($count==0){
		echo "<li> Книг по данному автору не найдено. Попробуйте снова. </li>";}
	else{
		echo "<h1>Книги по автору $criteria</h1>";
		echo "<table border=1 cellspacing=0 cellpadding=3 style='width:680px;'>";
...
elseif ($_GET['option'] == "publisher"){
	...
	// If result matched, table row must be greater than 1 row
	if($count==0){
		echo "<li> Книг по данному публикатору не найдено. Попробуйте снова. </li>";
	}
	else{
		$sn=1;
...
$sn=1;
if($count==0){
	echo "<li> Книг по данной категории не найдено. Попробуйте снова. </li>";
}
else{
	echo "<h1>Книги по категории $criteria</h1>";
	echo "<table border=1 cellspacing=0 cellpadding=3 style='width:680px;'>";
	echo "<tr><td>№</td><td>Название книги</td><td> Автор(ы)</td><td> Публикатор </td></tr>";
				...
				if($count6>=1){
					while($row6=mysql_fetch_array($result6)){
						...
						while($row4=mysql_fetch_array($result4)){
							...
							while($row5=mysql_fetch_array($result5)){
								$author_name= $row5['author_name'];
								echo "</td><td><img src='list.png'> $author_name<br>";
							}
						}
